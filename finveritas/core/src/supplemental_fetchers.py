...
SUPPLEMENTAL_FIELDS: frozenset[str] = frozenset({
    ...
    "retained_earnings",
    # V2 additions — both providers report these on the same balance-sheet call.
    "inventory",
    "accounts_receivable",
    "accounts_payable",
})
...
# FMP retired the legacy `/api/v3/<statement>/<symbol>` path-param endpoints
# (403 "Legacy Endpoint" as of the August 2025 API migration) in favour of
# `/stable/<statement>?symbol=<symbol>` query-param endpoints.
...
def fetch_missing_from_fmp(
    ...
    params = {"symbol": ticker, "period": "annual", "limit": limit, "apikey": fmp_api_key}
