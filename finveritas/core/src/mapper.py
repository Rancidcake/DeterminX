...
_FIELD_MAP: dict[str, list[re.Pattern[str]]] = {
    ...
    "amortization": [
        # Only matches when amortization is reported as its own line (not
        # combined into "Depreciation and Amortization" above).
        re.compile(r"^amortization(?:\s+expense)?$", re.IGNORECASE),
    ],
    "sga_expense": [
        re.compile(r"^selling,?\s*general\s*(?:&|and)\s*administrative", re.IGNORECASE),
        re.compile(r"^SG&A$", re.IGNORECASE),
        re.compile(r"^SGA$", re.IGNORECASE),
    ],
    "rd_expense": [
        re.compile(r"^research\s*(?:&|and)\s*development", re.IGNORECASE),
        re.compile(r"^R&D$", re.IGNORECASE),
    ],

    # ── Cash Flow Statement fields ───────────────────────────────────────
    "operating_cash_flow": [
        re.compile(r"^(?:net\s+)?cash\s+(?:flow\s+)?(?:from|generated\s+(?:from|by))\s+operat", re.IGNORECASE),
        re.compile(r"^operating\s+cash\s+flow", re.IGNORECASE),
        re.compile(r"^cash\s+from\s+operations", re.IGNORECASE),
    ],
    "investing_cash_flow": [
        re.compile(r"^(?:net\s+)?cash\s+(?:flow\s+)?(?:from|used\s+in)\s+invest", re.IGNORECASE),
        re.compile(r"^investing\s+cash\s+flow", re.IGNORECASE),
    ],
    "financing_cash_flow": [
        re.compile(r"^(?:net\s+)?cash\s+(?:flow\s+)?(?:from|used\s+in)\s+financ", re.IGNORECASE),
        re.compile(r"^financing\s+cash\s+flow", re.IGNORECASE),
    ],
    "capex": [
        re.compile(r"^capital\s+expenditure", re.IGNORECASE),
        re.compile(r"^capex$", re.IGNORECASE),
        re.compile(r"^purchase\s+of\s+(?:property|fixed\s+assets|ppe)", re.IGNORECASE),
    ],
    ...
    "inventory": [
        ...
        re.compile(r"^stock[\-\s]?in[\-\s]?trade", re.IGNORECASE),
    ],
    "accounts_receivable": [
        re.compile(r"^(?:trade\s+)?(?:accounts?\s+)?receivables?", re.IGNORECASE),
        re.compile(r"^debtors", re.IGNORECASE),
        re.compile(r"^sundry\s+debtors", re.IGNORECASE),
    ],
    "accounts_payable": [
        re.compile(r"^(?:trade\s+)?(?:accounts?\s+)?payables?", re.IGNORECASE),
        re.compile(r"^creditors", re.IGNORECASE),
        re.compile(r"^sundry\s+creditors", re.IGNORECASE),
    ],
}
