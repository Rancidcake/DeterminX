...
def _provider_defaults(provider: str) -> tuple[str, str, str]:
    ...
    When `.env` already configures this exact provider (LLM_PROVIDER matches),
    prefer the env-supplied LLM_BASE_URL/LLM_MODEL over the hardcoded literal
    below — hosted-model lineups (Groq, Gemini, ...) change over time and a
    stale hardcoded model name here would otherwise silently clobber a
    correct, user-configured `.env` value every time the sidebar re-seeds
    session state (see `main()`).
    """
    provider = (provider or "").strip().lower()
    _env_provider = (_DEFAULT_PROVIDER or "").strip().lower()
    _env_matches = _env_provider == provider

    if provider == "gemini":
        return (
            _DEFAULT_BASE_URL if _env_matches else "https://generativelanguage.googleapis.com/v1beta/openai/",
            _DEFAULT_MODEL if _env_matches else "gemini-2.0-flash",
            "Google AI Studio key",
        )
    if provider == "groq":
        # "llama-3.3-70b-versatile" was retired from Groq's model lineup;
        # kept only as a last-resort literal when nothing else is configured.
        return (
...
def _delete_session_data() -> None:
    ...
    for p in paths:
        try:
            ...
            pass
    for key in [
        "ocr_cache", "agent_outputs", "supp_prefilled", "show_supplement", "confirm_delete",
        "fact_ledger_cache", "fc_validation", "fc_forecast", "competitive_result",
        "credit_result", "geo_result", "geo_manual_data", "geo_scenario_parsed",
        "gov_result", "gov_manual_directors",
    ]:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        page = st.radio("",
            [
                "Upload Statement", "Agent Workflow", "Financial Analysis",
                "Financial Metrics", "Forecast", "Competitive Intelligence",
                "Risk & Alerts", "Credit Intelligence", "Geography", "Governance",
                "Basel III Alignment", "Compliance & Audit",
            ],
            label_visibility="collapsed")
    ...
    if page == "Upload Statement":
        ...
        v2_pages.page_financial_metrics()
    elif page == "Forecast":
        v2_pages.page_forecast(base_url, model, api_key)
    elif page == "Competitive Intelligence":
        v2_pages.page_competitive(base_url, model, api_key)
    elif page == "Risk & Alerts":
        v2_pages.page_risk_alerts()
    elif page == "Credit Intelligence":
        v2_pages.page_credit(base_url, model, api_key)
    elif page == "Geography":
        v2_pages.page_geography(base_url, model, api_key, news_api_key)
    elif page == "Governance":
        v2_pages.page_governance(base_url, model, api_key, news_api_key)
    elif page == "Basel III Alignment":
        page_basel()
