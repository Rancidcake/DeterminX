...
## [V2] — Explainable Financial Intelligence: Metrics Engine, Forecasting, Risk, Competitive, Credit, Geography, Governance

V1's five-agent historical-analysis pipeline is preserved unchanged. V2 adds a new deterministic
foundation and seven new capabilities on top of it, per the V2 architecture brief. The central rule
carries through unchanged: **Python computes, the LLM only narrates — no financial number is ever
produced by the LLM.**

### Deterministic Financial Metrics Engine (`metrics/`) — new

Centralised Python engine covering the 12 metric categories: Revenue, Cost, Profitability, Liquidity,
Solvency, Debt Servicing, Efficiency, Returns, Cash Flow, Growth, Trend, and Risk Intelligence.
`metrics/engine.py::build_fact_ledger(payload)` runs all 12 category modules and returns an immutable
`FactLedger` of `Fact` objects (`metrics/schema.py`) — each with metric, value, unit, period, source,
formula, and an explicit availability status:

```
CALCULATED · ESTIMATED · UNAVAILABLE · REQUIRES_EXTERNAL_DATA · REQUIRES_ADDITIONAL_DOCUMENT
```

A metric is only ever ESTIMATED when a disclosed, documented simplifying assumption is used (e.g.
Tandon Method II 25% minimum NWC); it is never a guess. DSCR and Drawing Power are always
REQUIRES_ADDITIONAL_DOCUMENT unless a debt repayment schedule / bank margin data is supplied — they
are never fabricated from the income statement alone. The engine consumes the same `entity` +
`time_series` payload V1 already produces; no ingestion pipeline was replaced.

**Ingestion schema extended** (`src/mapper.py`, `src/yfinance_ingestion.py`,
`src/private_company_ingestion.py`) with new canonical fields the engine needs: `sga_expense`,
`rd_expense`, `amortization`, `inventory`, `accounts_receivable`, `accounts_payable`,
`operating_cash_flow`, `investing_cash_flow`, `financing_cash_flow`, `capex`. All existing V1 field
mappings are untouched — this is purely additive.

### External Data Layer (`src/external/`) — new

Provider abstraction (`ExternalDataProvider` / `ProviderResult`) behind which every external
dependency sits: peer financials (`peer_data_provider.py`, reuses `yfinance_ingestion`), management
guidance (`guidance_provider.py`, user-entered), macro indicators (`macro_provider.py`, live World
Bank Open Data, free/keyless), analyst estimates and MCA/SEBI/IBBI governance registries
(`analyst_estimates_provider.py`, `governance_provider.py` — both `NOT_CONFIGURED`, no licensed feed
available in this deployment), and news (`news_provider.py`, wraps the existing NewsAPI client from
`src/sentiment_agent.py`). Every result carries `status`/`source`/`retrieved_at` — nothing is ever
silently fabricated when a source is unavailable.

### Forecast — Assumption Validation Agent (`src/forecast/`) — new

User enters a growth assumption; Python deterministically compares it against historical CAGR,
management guidance, and a sector benchmark to produce a verdict (`ALIGNED` / `OPTIMISTIC` /
`AGGRESSIVE` / `CAUTIOUS` / `OVERLY_CONSERVATIVE`) — the LLM only explains *why*, never changes the
verdict. `forecast_engine.py` then projects the metric forward using the validated growth rate (pure
compounding, no LLM arithmetic) and asks the LLM for a causal "why" narrative built only from
deterministically-derived growth-driver observations.

### Competitive Intelligence (`src/competitive/`) — new

Peer tickers are fetched via the existing yfinance ingestion path and run through the same metrics
engine as the target, so peer numbers are exactly as auditable as the target's own.
`peer_benchmarking.py` computes target vs. peer-median/average/gap for Revenue CAGR, EBITDA, EBITDA
Margin, PAT CAGR, ROE, ROCE, Debt-to-Equity, margins, and asset turnover — all in Python; the LLM only
narrates the resulting gaps.

### Risk & Anomaly Alert System (`src/risk_monitoring/`) — new

Extends the existing Credibility Engine's PASS/WARN/FAIL pattern into continuous monitoring:
`anomaly_engine.py` compares each structural ratio's current-period value against the company's own
historical mean/range (z-score based) and flags PASS/WARN/FAIL — runs on demand (whenever analysis is
requested), no scheduler/real-time infrastructure added. `metrics/risk.py` (part of the metrics
engine) separately covers the absolute-threshold risk checks: negative EBITDA, declining PAT, margin
compression, rising debt, weak current ratio / interest coverage / DSCR, negative operating cash flow,
abnormal receivable growth.

### Credit Intelligence — Working Capital Agent (`src/credit/`) — new

Tandon Method II style working-capital assessment (`working_capital_agent.py`): Gross/Net Working
Capital, Working Capital Gap, Minimum Stipulated NWC (25% of TCA), Borrower Contribution, Estimated
MPBF, Current Asset Coverage Ratio, Operating Cycle, DSO/DIO/DPO, Cash Conversion Cycle. MPBF is always
labelled ESTIMATED; Drawing Power is always REQUIRES_ADDITIONAL_DOCUMENT (needs bank sanction-letter
margin percentages this system does not ingest) — never presented as bank-grade.

### Geography-wise Impact Analysis (`src/geography/`) — new

Regional revenue, share, YoY growth, and volatility are 100% Python-computed from user-entered figures
(with best-effort regex extraction from pasted segment-note text, `geography_extractor.py`, as a
starting point). For regions with a notable swing, recent headlines (via the existing NewsAPI client)
are shown to the LLM, which must separate its output into a FACT section (the computed numbers) and a
POSSIBLE EXPLANATION section (hedged, never presented as proven).

### Governance — Executive Background Agent (`src/governance/`) — new

Best-effort DIN/director extraction from pasted filing text (`din_extractor.py`), a statutory-registry
lookup via the `NOT_CONFIGURED` governance provider, and a deterministic keyword-based adverse-mention
screen over recent news headlines (`governance_agent.py`) — a keyword match is reported as "requires
manual verification," never as an accusation. Governance status is always reported separately from
financial health, never blended into one score.

### New UI pages (`ui/v2_pages.py`, `app.py`)

Seven new sidebar pages — **Financial Metrics**, **Forecast**, **Competitive Intelligence**,
**Risk & Alerts**, **Credit Intelligence**, **Geography**, **Governance** — added alongside the
unmodified V1 pages. Every status (CALCULATED/ESTIMATED/UNAVAILABLE/REQUIRES_EXTERNAL_DATA/
REQUIRES_ADDITIONAL_DOCUMENT/PASS/WARN/FAIL) is rendered as an explicit badge so the UI never implies
a number exists when it doesn't. All V2 agent calls are wrapped the same way V1's pipeline wraps
agent calls (`_safe_call`, mirroring `app.py::_safe_run`) — an unreachable LLM endpoint surfaces as an
`st.error`, never a crash.
